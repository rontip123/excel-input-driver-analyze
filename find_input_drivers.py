...
def is_text_only(value):
    ...
    if isinstance(value, str):
        ...
        value_with_placeholder = re.sub(
            r"(?<=[a-zA-Z])-(?=[a-zA-Z])|(?<=\s)-(?=\s)", "HYPHEN_PLACEHOLDER", value
        )
...
def find_row_label_xlrd(sheet, row, start_col, workbook):
    ...
    try:
        ...
        for col in range(start_col, -1, -1):
            ...
            print(
                f"  Checking column {get_column_letter(col+1)}: value='{cell_value}', is_text={is_text_only_xlrd(cell, workbook)}"
            )
            if cell.ctype != xlrd.XL_CELL_EMPTY and is_text_only_xlrd(cell, workbook):
                print(
                    f"  Found row label: '{cell_value}' at column {get_column_letter(col+1)}"
                )
                return cell_value
        ...
        if file_ext == ".xls":
            ...
            for cell_ref in cells_to_check:
                try:
                    ...
                    column_label = find_column_label_xlrd(
                        sheet, row - 1, col_idx, workbook
                    )
                ...
                for cell_ref in referenced_cells:
                    try:
                        ...
                        is_input = cell.value is not None and (
                            ...
                            or is_number_only_formula(cell.value)
                            or is_input_driver_formula(cell.value)
                        )
                        ...
                        if is_input:
                            ...
                            if row_label:
                                ...
                                column_label = find_column_label(
                                    sheet, row - 1, col_idx
                                )
            ...
            for sheet_name, input_drivers in all_input_drivers_by_sheet.items():
                ...
                max_cols = [
                    col for col, count in col_counts.items() if count == max_count
                ]
                ...
                for (
                    ...
                    if (
                    ...
                    ):  # Only process cells in the selected column
                        ...
                        first_right_col_label = find_column_label(
                            sheet, row, first_right_col
                        )
                        second_right_col_label = find_column_label(
                            sheet, row, second_right_col
                        )
